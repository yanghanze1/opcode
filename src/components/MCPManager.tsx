...
export const MCPManager: React.FC<MCPManagerProps> = ({
  ...
  const handleServerAdded = () => {
    loadServers();
    setToast({ message: t('messages.server_added'), type: "success" });
  ...
  const handleServerRemoved = (name: string) => {
    ...
    setToast({ message: t('messages.server_removed', { name }), type: "success" });
  };
  ...
  const handleImportCompleted = (imported: number, failed: number) => {
    ...
    if (failed === 0) {
      setToast({
        message: t('messages.servers_imported', { count: imported }),
        type: "success"
      });
    } else {
      setToast({
        message: t('messages.servers_imported_with_errors', { imported, failed }),
        type: "error"
      });
    }
  };
