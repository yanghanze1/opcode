...
export const ClaudeFileEditor: React.FC<ClaudeFileEditorProps> = ({
  ...
  const handleSave = async () => {
    try {
      ...
      setToast({ message: t('claude_file_editor.saved_successfully'), type: "success" });
    } catch (err) {
      console.error("Failed to save file:", err);
      setError(t('claude_file_editor.failed_to_save'));
      setToast({ message: t('claude_file_editor.failed_to_save'), type: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBack = () => {
    if (hasChanges) {
      const confirmLeave = window.confirm(
        t('claude_file_editor.unsaved_changes_warning')
      );
