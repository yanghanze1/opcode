...
export const CreateAgent: React.FC<CreateAgentProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (!systemPrompt.trim()) {
      setError(t('validation.prompt_required'));
      return;
    }
    ...
    try {
      ...
      const errorMsg = isEditMode ? t('messages.failed_to_update') : t('messages.failed_to_create');
      setError(errorMsg);
      setToast({
        message: errorMsg,
        type: "error"
  ...
  const handleBack = () => {
    if ((name !== (agent?.name || "") ||
        ...
        !confirm(t('messages.unsaved_changes'))) {
      return;
    }
    onBack();
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-heading-1">
                  {isEditMode ? t('labels.edit_agent') : t('labels.create_new_agent')}
                </h1>
                <p className="mt-1 text-body-small text-muted-foreground">
                  {isEditMode ? t('labels.update_agent_config') : t('labels.configure_new_agent')}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-5">
              <div className="flex items-center gap-2 mb-4">
                <h3 className="text-heading-4">{t('labels.basic_information')}</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-caption text-muted-foreground">{t('labels.agent_name')}</Label>
                  <Input
                    id="name"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder={t('placeholders.agent_name')}
                    required
                    className="h-9"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label className="text-caption text-muted-foreground">{t('labels.agent_icon')}</Label>
                  <motion.div
                    whileTap={{ scale: 0.97 }}
                    transition={{ duration: 0.15 }}
              ...
              <div className="space-y-2 mt-4">
                <Label className="text-caption text-muted-foreground">{t('labels.model')}</Label>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-caption text-muted-foreground">{t('models.sonnet.description')}</div>
                      </div>
                    </div>
                  </motion.button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-caption text-muted-foreground">{t('models.opus.description')}</div>
                      </div>
                    </div>
                  </motion.button>
                </div>
            ...
            {/* Configuration */}
            <Card className="p-5">
              <h3 className="text-heading-4 mb-4">{t('labels.configuration')}</h3>
              <div className="space-y-2">
                <Label htmlFor="default-task" className="text-caption text-muted-foreground">{t('labels.default_task')}</Label>
                <Input
                  id="default-task"
                  type="text"
                  placeholder={t('placeholders.default_task')}
                  value={defaultTask}
            ...
            <Card className="p-5">
              <div className="mb-4">
                <h3 className="text-heading-4 mb-1">{t('labels.system_prompt')}</h3>
                <p className="text-caption text-muted-foreground">
