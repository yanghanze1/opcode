...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('messages.agent_created'), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('messages.agent_updated'), type: "success" });
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : t('messages.failed_to_import');
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-heading-4 mb-2">{t('messages.no_agents')}</h3>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-caption text-muted-foreground">
                                  {t('labels.created')}: {new Date(agent.created_at).toLocaleDateString()}
                                </p>
                              </CardContent>
              ...
              {!loading && agents.length > 0 && (
                <div className="overflow-hidden">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-heading-4">{t('execution.recent_executions')}</h2>
                  </div>
                  {runsLoading ? (
